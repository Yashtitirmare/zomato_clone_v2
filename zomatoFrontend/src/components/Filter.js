...
export default function Filter() {
  ...
  useEffect(() => {
    //fetch
    fetch(
      `${process.env.REACT_APP_API_URL}/restaurant/filter/${currentPageNo}`,
      requestOption
    )
      ...
      .then((data) => {
        ...
        if (data?.totalRecords) setPageCount(Math.ceil(data.totalRecords / 2));
      });
  }, [filter, currentPageNo]);

  useEffect(() => {
    fetch(`${process.env.REACT_APP_API_URL}/location`, { method: "GET" })
      .then((response) => response.json())
      .then((data) => {
  ...
  //   };
  const handleCuisineChange = (event) => {
    const updatedCuisine = [...filter.cuisine];
    if (event.target.checked) {
      updatedCuisine.push(event.target.name);
    } else {
      const index = updatedCuisine.indexOf(event.target.name);
      if (index > -1) updatedCuisine.splice(index, 1);
    }
    setFilter({ ...filter, cuisine: updatedCuisine });
  };
  ...
  for (let i = 1; i <= pageCount; i++) {
    paginationItems.push(
      //   <a key={i} href="#" onClick={() => setCurrentPageNo(i)}>
      //     {i}
      //   </a>
      <button key={i} onClick={() => setCurrentPageNo(i)}>
        {i}
      </button>
    );
  }
              ...
              <div id="demo" className="collapse show">
                ...
                <div>
                  <input
                    type="radio"
                    name="cost"
                    onChange={() => handleCostChange(2000, 3000)}
                  />
                ...
                <div>
                  <input
                    type="radio"
                    name="cost"
                    onChange={() => handleCostChange(1, 3000)}
                  />
                  ...
                  <div className="row padding-left">
                    <div className="col-sm-12 col-md-12 col-lg-12">
                      <div className="rest-address">
                        CUISINES :{" "}
                        {item.Cuisine.length &&
                          item.Cuisine.map((item) => item.name + " ")}
                      </div>
                      <div className="rest-address">
