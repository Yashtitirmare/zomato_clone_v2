...
export default function RestaurantDetail() {
  ...
  useEffect(()=>{
    fetch(`${process.env.REACT_APP_API_URL}/restaurant/details/${rName}`,{method:'GET'})
    .then(response=>response.json())
    .then(data=>setRestaurant(data.data))
  },[])
  
  const fetchMenu = ()=>{
    fetch(`${process.env.REACT_APP_API_URL}/menu/${rName}`,{method:'GET'})
    .then(response=>response.json())
  ...
  const openRazorpay=async()=>{
   try{ //create order in razorpay by calling backend API
    let orderData;
    orderData = await fetch(`${process.env.REACT_APP_API_URL}/pay`,{
      method:'POST',
...
//open razorpay window
    const options ={
       ...
       handler:function(response){
        ...
        fetch(`${process.env.REACT_APP_API_URL}/pay/save`,{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({
